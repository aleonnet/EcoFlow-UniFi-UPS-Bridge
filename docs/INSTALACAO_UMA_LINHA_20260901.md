...
| 05 Verificação | espera a API local, lê `/v1/version` e `/v1/health`; **falha (código 1)** se o serviço não subir ou se quem responde não for o código instalado, nomeando o programa (sem PID na tela; o PID vai ao log). Ao final de qualquer falha: *Feito até aqui / Faltou / O que fazer agora* | — |
