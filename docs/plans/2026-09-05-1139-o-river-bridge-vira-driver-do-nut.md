...
| `river` | o River completo: bateria, autonomia, situação, tensão, potência total, **watts por tomada**, frequência, temperatura | `load.off` (desliga o River) |
...
| D5 | Comandos **só com nomes da lista fechada do Home Assistant**: `load.off` no `river`; `load.off` e `shutdown.reboot` em cada dispositivo protegido. | §2.2. |
| D6 | As cercas dos comandos são **as mesmas da tela**: `load.off` do River exige a trava de arquivo aberta e nenhuma proteção armada; o desligamento de um dispositivo exige alcance provado. A recusa volta como `TRACKING <id> <código>` e vira linha no registro. | Um comando pelo NUT não pode ter menos cerca que o mesmo comando pelo App. |
...
| # | Commit | Estado |
|---|---|---|
| C1 | driver + publicação + testes | **feito** (26 testes) |
| C2 | o serviço publicando, e a recusa de D3 (S31) | |
| C3 | comandos com as cercas do serviço (S32–S34) | |
| C4 | validade da leitura serial na publicação (S35) | |
| C5 | empréstimo automático do cabo (S36, S37) + aviso no App | |
| C6 | `ups.conf` e `upsd.users` no instalador | |
| C7 | tela "Serviço", DMG, versão 0.7.0 | |
| C8 | bancada do dono | |

Revisão fria sobre o diff depois de C4 e depois de C7.
...
- **Usuário:** `homeassistant` · **Senha:** a gerada na instalação (mostrada no App)
- Escolher o aparelho **`river`** — e repetir a integração escolhendo **`udr7`** para ter o
  roteador como aparelho próprio
...
- **Contra o NUT de verdade**, no Mac mini: `upsc river@127.0.0.1` listando os watts por tomada
  e `upscmd -l river@127.0.0.1` listando os comandos.
